...
class _TournamentEntryFormScreenState
  ...
  // ✅ 커스텀 질문용 컨트롤러 (팀원들용: List index별로 질문 Map 관리)
  ...
  int _currentTeamSize = 2; // 기본 2명부터 시작
  ...
  void dispose() {
    ...
    for (var map in _memberCustomCtrls) {
      ...
      if (newCount >= 2 && newCount <= maxTeamSize) {
        ...
        if (delta > 0) {
          ...
          for (var q in questions) {
            newMap[q] = TextEditingController();
          }
  ...
  void _initControllers(TournamentModel t) {
    if (_memberNames.isNotEmpty) return;

    // 팀장 질문 컨트롤러 세팅
    for (var q in t.customQuestions) {
      _leaderCustomCtrls[q] = TextEditingController();
    }
    ...
    if (t.type == 'team') {
      ...
      for (var q in t.customQuestions) {
        firstMemberMap[q] = TextEditingController();
      }
  ...
  Future<void> _submit(TournamentModel t, User user) async {
    ...
    try {
      ...
      _leaderCustomCtrls.forEach((key, ctrl) {
        leaderAnswers[key] = ctrl.text.trim();
      });
      ...
      if (t.type == 'team') {
        for (int i = 0; i < _memberNames.length; i++) {
          ...
          _memberCustomCtrls[i].forEach((key, ctrl) {
            memberAnswers[key] = ctrl.text.trim();
          });

          members.add(TeamMember(
            name: _memberNames[i].text.trim(),
            rating: _memberRatings[i].text.trim(),
            customAnswers: memberAnswers, // ✅ 팀원별 답변 저장
          ));
      ...
      final entry = TournamentEntryModel(
        ...
        customAnswers: leaderAnswers, // ✅ 팀장 답변 저장
      ...
      child: Column(
        ...
        children: [
          ...
          Text(isPaid ? '참가비 입금이 확인되었습니다.\n대회 당일 현장에서 뵙겠습니다!' : '신청서가 정상 접수되었습니다.\n주최자가 입금을 확인하면 "입금완료"로 변경됩니다.',
              textAlign: TextAlign.center, style: TextStyle(color: Colors.grey[600], height: 1.5, fontSize: 14)),
    ...
    return Form(
      ...
      child: ListView(
        ...
        children: [
          ...
          Text(t.type == 'team' ? "팀 참가 신청 정보를 입력해 주세요." : "개인 참가 신청 정보를 입력해 주세요.",
          ...
          AppCard(
            ...
            child: Column(
              children: [
                if (t.type == 'team') ...[
                  _buildField("팀명 *", _teamNameCtrl, Icons.groups_outlined),
                ...
                // ✅ 팀장용 추가 질문들
                if (t.customQuestions.isNotEmpty) ...[
                  const Divider(height: 32),
                  ...t.customQuestions.map((q) => Padding(
                    padding: const EdgeInsets.only(bottom: 12),
                    child: _buildField(q, _leaderCustomCtrls[q]!, Icons.help_outline),
                  )),
                ],
              ],
            ),
          ...
          if (t.type == 'team') ...[
            ...
            Row(
              ...
              children: [
                const Text("팀원 정보", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.cyan)),
                Container(
                  decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(20)),
                  child: Row(
                    children: [
                      IconButton(
                        icon: const Icon(Icons.remove_circle_outline, size: 20, color: Colors.cyan),
                        onPressed: () => _updateMemberCount(-1, t.teamSize, t.customQuestions),
                      ),
                      Text('$_currentTeamSize명', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                      IconButton(
                        icon: const Icon(Icons.add_circle_outline, size: 20, color: Colors.cyan),
                        onPressed: () => _updateMemberCount(1, t.teamSize, t.customQuestions),
                      ),
                    ],
                  ),
            ...
            const SizedBox(height: 12),
            ...List.generate(_memberNames.length, (index) => Padding(
              ...
              child: AppCard(
                ...
                child: Column(
                  ...
                  children: [
                    Text("팀원 ${index + 2}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)),
                    const SizedBox(height: 12),
                    _buildField("이름(한글) *", _memberNames[index], Icons.person_add_alt_1_outlined),
                    const SizedBox(height: 16),
                    _buildField("레이팅 *", _memberRatings[index], Icons.star_border_outlined, isPhone: true),
                    ...
                    if (t.customQuestions.isNotEmpty) ...[
                      ...
                      ...t.customQuestions.map((q) => Padding(
                        padding: const EdgeInsets.only(bottom: 12),
                        child: _buildField(q, _memberCustomCtrls[index][q]!, Icons.help_outline),
                      )),
                    ],
                  ],
                ),

...
class SteelLeagueScheduleScreenBody extends ConsumerWidget {
            ...
            children: [
                  ...
                  child: TableCalendar(
                    ...
                    calendarBuilders: CalendarBuilders(
                      ...
                      todayBuilder: (context, day, focusedDay) {
                        if (isSameDay(day, calendarState.selectedDay)) {
                          return null;
                        }
                        return Center(
                          child: Container(
              ...
              Expanded(
                child: calendarState.selectedDay == null
                  ...
                  itemCount: calendarNotifier
                      .getEventsForDay(
                      calendarState.selectedDay!)
                      .length,
                  itemBuilder: (_, i) {
                    final event = calendarNotifier.getEventsForDay(
                        calendarState.selectedDay!)[i];
                    final eventDateTime =
                    event['eventDateTime'] as DateTime;
                    final status =
                    _getEventStatus(eventDateTime);

                    final winner =
                    event['winner'] as String?;
                    final hasWinner = winner != null &&
                        winner.trim().isNotEmpty;
                    final hasImage =
                        event['resultImageUrl'] != null;
  ...
  void _showEventDetail(
      BuildContext context, Map<String, dynamic> event, DateTime eventDateTime) {
    final status = _getEventStatus(eventDateTime);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildCompactRow(
                          ...
                          '${event['entryFee'] ?? 0}원'),
                      const SizedBox(height: 12),
                      _buildCompactRow(context, '관리자',
                          event['admin'] ?? '미정'),
                      _buildCompactRow(context, '연락처',
                          event['contact'] ?? '미정'),
                      const SizedBox(height: 12),
                      if (status == 'completed') ...[
                        _buildCompactRow(context, '상태', '종료됨',
                            color: Colors.red),
                        if (hasWinner)
                          Padding(
                            padding:

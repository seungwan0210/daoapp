...
class _TournamentCreateScreenState extends ConsumerState<TournamentCreateScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final int maxParticipants = (maxText.toLowerCase() == '무제한' || maxText == '0')
          ? 9999
      ...
      final tournament = TournamentModel(
        ...
        organizerEmails: [user.email ?? '', ..._coOrganizers],
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context); // ← 여기서 선언!
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('대회 개설', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
        elevation: 0,
        backgroundColor: theme.scaffoldBackgroundColor,
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                GestureDetector(
                  onTap: _pickPoster,
                  child: _posterFile != null ? _buildPosterPreview(theme) : _buildPosterPlaceholder(theme),
                ),
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        validator: (v) {
                          ...
                          final fee = int.tryParse(v.replaceAll(',', ''));
                          if (fee == null || fee < 1000) return '1,000원 이상';
                          if (fee % 1000 != 0) return '1,000원 단위로 입력';
                          return null;
                        },
                      ),
                    ),
                ...
                Container(
                  ...
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Icon(Icons.lightbulb, color: theme.colorScheme.primary, size: 26),
                      const SizedBox(width: 14),
                      Expanded(
                        child: RichText(
                          text: TextSpan(
                            style: TextStyle(fontSize: 15, height: 1.6, color: theme.colorScheme.onBackground),
                            children: [
                              const TextSpan(text: '추천 설정\n', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16.5)),
                              const TextSpan(text: '• 엔트리 시작: '),
                              TextSpan(text: '대회 10일 전\n', style: TextStyle(fontWeight: FontWeight.bold, color: theme.colorScheme.primary)),
                              const TextSpan(text: '   → 참가자 모집 기간을 넉넉히 확보할 수 있어요\n'),
                              const TextSpan(text: '• 엔트리 마감: '),
                              TextSpan(text: '대회 3일 전\n', style: TextStyle(fontWeight: FontWeight.bold, color: theme.colorScheme.primary)),
                              const TextSpan(text: '   → 명단 정리·이메일 발송·대진표 제작에 충분한 여유를 줍니다'),
                            ],
                          ),
                        ),
                      ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.mark_email_read_outlined, color: Colors.blue[700], size: 24),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Text(
                ...
                _buildDateTile('엔트리 시작 (자동)', _entryStartDate, (d) => setState(() => _entryStartDate = d)),
                const SizedBox(height: 12),
                _buildDateTile('엔트리 마감 (자동)', _entryEndDate, (d) => setState(() => _entryEndDate = d)),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(Icons.add_a_photo_outlined, size: 42, color: theme.colorScheme.primary),
          ),
          const SizedBox(height: 16),
          Text("포스터 추가하기", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: theme.colorScheme.primary)),
          const SizedBox(height: 6),
          Text("터치해서 사진을 선택하세요", style: TextStyle(color: Colors.grey[600], fontSize: 14)),
        ],
      ),
    );
  }
  ...
  Widget _buildPosterPreview(ThemeData theme) {
    return Stack(
      children: [
        ClipRRect(
          borderRadius: BorderRadius.circular(20),
          child: Image.file(_posterFile!, width: double.infinity, height: 340, fit: BoxFit.cover),
        ),
        Positioned(
          top: 12,
          right: 12,
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  Text("변경", style: TextStyle(fontWeight: FontWeight.bold, color: theme.colorScheme.primary, fontSize: 15)),
                ],
              ),
            ),
          ),
      ...
      child: ListTile(
        ...
        subtitle: Padding(
          ...
          child: Text(
            date != null
                ...
                : '탭하여 날짜 선택',
            style: TextStyle(fontSize: 15, color: date != null ? Colors.black87 : Colors.grey[500]),
          ),
        ),
        ...
        onTap: () async {
          ...
          if (picked != null) {
            ...
            if (time != null) {
              onSelect(picked.copyWith(hour: time.hour, minute: time.minute));
            }
          }
        },
      ),
...
class _CoOrganizerInputState extends State<_CoOrganizerInput> {
          ...
          children: [
            Row(
              children: [
                ...
                Text('공동주최자 이메일 추가 (선택)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: theme.colorScheme.primary)),
              ],
            ),
            const SizedBox(height: 16),
            TextField(
  ...
  void _addEmail() {
    ...
    if (email.isNotEmpty &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('올바른 이메일 주소를 입력해주세요'), behavior: SnackBarBehavior.floating),
      );
    }
  }
}
...
class CheckoutPracticeDetailScreen extends StatelessWidget {
  ...
  const CheckoutPracticeDetailScreen({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final successRate =
        (data['successRate'] as num?)?.toDouble() ?? 0.0; // 0~1
    final avgDarts =
    ...
    if (successCount == 0 && problemCount > 0) {
      successCount = (successRate * problemCount).round();
    }

    // ✅ 새로 추가된 지표(없으면 0으로 처리)
    final optimizationRate =
        (data['optimizationRate'] as num?)?.toDouble() ?? 0.0; // 0~1
    final routeMatchRate =
        (data['routeMatchRate'] as num?)?.toDouble() ?? 0.0;   // 0~1
    final score =
        (data['score'] as num?)?.toInt() ?? 0;

    // ✅ 문제별 상세 (새 구조: usedSegments / recommendedRoute)
    final problemsRaw =
        data['problems'] as List<dynamic>? ?? const [];
    final problems =
    problemsRaw.cast<Map<String, dynamic>>();
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildStatRow(
                      label: '최적 다트율',
                      value:
                      '${(optimizationRate * 100).toStringAsFixed(0)} %',
                    ),
                    _buildStatRow(
                      label: '정석 루트율',
                      value:
                      '${(routeMatchRate * 100).toStringAsFixed(0)} %',
                    ),
                    _buildStatRow(
                      label: '세션 점수',
                    ...
                    children: [
                      ...
                      ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final usedSegments =
                              (p['usedSegments'] as List<dynamic>?)
                                  ?.cast<String>() ??
                                  const <String>[];
                            ...
                            subtitle: Column(
                              ...
                              children: [
                                ...
                                if (recommendedRoute
                                  ...
                                  const SizedBox(height: 2),
                                  Text(
                                    '추천 정석 루트: ${recommendedRoute.join(" → ")}',
                                    style: const TextStyle(
                                      fontSize: 12,
                                      color: Colors.blue,
                                    ),

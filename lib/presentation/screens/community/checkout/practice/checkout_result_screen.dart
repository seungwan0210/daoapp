...
class CheckoutResultScreen extends StatelessWidget {
  ...
  double _calculateOptimizationRate(List<PracticeResult> results) {
    ...
    for (final r in successResults) {
      ...
      if (r.dartsUsed == optimalDarts) optimalCount++;
    }
    return (optimalCount / successResults.length) * 100;
  }

  // 정석 루트율 계산 (성공한 문제 중에서 정석 루트(primary)와 정확히 일치한 비율)
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () => Navigator.of(context).popUntil(ModalRoute.withName(RouteConstants.checkoutHome)),
        ),
      ),
      body: FutureBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          final routeAccuracyRate = _calculateRouteAccuracyRate(args.results); // ← 정석 루트율 추가!
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        const Divider(height: 24),
                        _buildStatRow("정석 루트율", "${routeAccuracyRate.toStringAsFixed(0)}%", Icons.navigation), // ← 새로 추가!
                        const Divider(height: 24),
                ...
                const Text("문제별 결과", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return AppCard(
                      child: ExpansionTile(
                        ...
                        trailing: isSuccess ? Text("$usedDarts다트", style: const TextStyle(fontWeight: FontWeight.bold)) : null,
                        children: [
                          Padding(
                            padding: const EdgeInsets.all(16),
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 4),
                                Text(optimalRoute.isEmpty ? "없음" : optimalRoute.join(" → "), style: const TextStyle(fontSize: 16, color: Colors.blue)),
                                const SizedBox(height: 8),
                                if (isSuccess)
                                  Text(
                                    usedDarts == optimalDarts ? "최적 다트 수로 성공!" : "성공했지만 최적 다트 수는 $optimalDarts다트입니다.",
                                    style: TextStyle(color: usedDarts == optimalDarts ? Colors.green : Colors.orange, fontWeight: FontWeight.w500),
                                  )
                                else
                                  const Text("다음엔 최적 다트 수/루트를 도전해보세요!", style: TextStyle(color: Colors.red)),
                              ],
                            ),
                          ),
                        ],

...
class PointAwardScreen extends StatefulWidget {
  ...
  @override State<PointAwardScreen> createState() => _PointAwardScreenState();
}
...
class _PointAwardScreenState extends State<PointAwardScreen> {
        ...
        child: Column(
          children: [
            ...
            DropdownButtonFormField<String>(
              value: _phase,
              items: [
                DropdownMenuItem(value: 'season1', child: const Text('시즌 1')),
                DropdownMenuItem(value: 'season2', child: const Text('시즌 2')),
                DropdownMenuItem(value: 'season3', child: const Text('시즌 3')),
              ],
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: _searchResults.isNotEmpty
                    ? IconButton(icon: const Icon(Icons.clear), onPressed: () => setState(() => _searchResults = []))
                    : null,
              ),
            ...
            if (_isNewUser && _koreanName.isNotEmpty)
              Card(
                ...
                child: Padding(
                  ...
                  child: Text('신규 등록: $_koreanName', style: const TextStyle(color: Colors.green)),
                ),
              ),
            ...
            DropdownButtonFormField<String>(
              value: _gender,
              items: [
                DropdownMenuItem(value: 'male', child: const Text('남자')),
                DropdownMenuItem(value: 'female', child: const Text('여자')),
              ],
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  '포인트 부여',
                  style: TextStyle(fontSize: 18, color: _canAward() ? Colors.white : Colors.white70),
                ),
              ),
            ),
          ],
  ...
  bool _canAward() {
    ...
    if (_isNewUser) {
      return hasName && hasPoints && _englishController.text.isNotEmpty && _shopController.text.isNotEmpty;
    } else {
  ...
  Future<void> _award() async {
    ...
    await context.read<PointRecordRepository>().awardPoints(record);
    provider.Provider.of<RankingProvider>(context, listen: false).loadRanking();
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('포인트 부여 완료!')));

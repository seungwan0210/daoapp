...
class TransferManager {
    ...
    async uploadManyFiles(filePathsOrDirectory, options = {}) {
        ...
        for (const filePath of allPaths) {
            ...
            if (options.customDestinationBuilder) {
                passThroughOptionsCopy.destination = options.customDestinationBuilder(filePath, options);
            }
            else {
    ...
    async downloadManyFiles(filesOrFolder, options = {}) {
        ...
        for (const file of files) {
            ...
            if (path.isAbsolute(name)) {
                const absolutePathError = new file_js_1.RequestError(file_js_1.FileExceptionMessages.ABSOLUTE_FILE_NAME);
            ...
            if (finalPath !== baseDir && !finalPath.startsWith(normalizedBaseDir)) {
                const traversalError = new file_js_1.RequestError(file_js_1.FileExceptionMessages.TRAVERSAL_OUTSIDE_BASE);
     ...
     */
    _resolveAndValidateBaseDir(options) {
        var _a, _b;
        const cwd = process.cwd();
        // Resolve baseDir, defaulting to CWD if no destination is provided
        const baseDir = path.resolve((_b = (_a = options.passthroughOptions) === null || _a === void 0 ? void 0 : _a.destination) !== null && _b !== void 0 ? _b : cwd);
        ...
        if (relativeBaseDir.startsWith('..') || path.isAbsolute(relativeBaseDir)) {
            const traversalError = new file_js_1.RequestError(file_js_1.FileExceptionMessages.TRAVERSAL_OUTSIDE_BASE_DESTINATION);
            throw traversalError;
        }
        return baseDir;
    }
